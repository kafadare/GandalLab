...
data.batch <- c()
  ...
  for (i in 1:ncol(genExp[,which(colnames(genExp) %in% colnames(combat_expr))])) {
    sample <- colnames(genExp[,which(colnames(genExp) %in% colnames(combat_expr))])[i]
    #match the sample to id in meta file and find corresponding "study"
    data.batch[i] <- meta[which(meta$Subject %in% sample), "study"]
  } 

s <- svd(genExp[,which(colnames(genExp) %in% colnames(combat_expr))] - rowMeans(genExp[,which(colnames(genExp) %in% colnames(combat_expr))]))
PC1 <- s$d[1]*s$v[,1]
PC2 <- s$d[2]*s$v[,2]
...
covs_filter <- known_covs %>% select(age, sex, paste0("genoPC", 1:20, sep = "")) #use age rather than logPcd, because age is better distributed
