...
#Load both fetal and adult, combine datasets

```{r}
fetal_data <- read.delim("/u/project/gandalm/cindywen/isoform_twas/salmon/expression.final/gene.noVersion.scaled.counts.tsv")
fetal_data$source <- factor(rep('F', nrow(fetal_data)))
adult_data <- readRDS("/u/scratch/d/danieldu/renormalize_0429/AdultBigBrain_gene_exp_raw_042923.RDS")
adult_data$source <- factor(rep('A', nrow(adult_data)))
##change this location to correct location
datExpr <- merge(fetal_data, adult_data, by.x =, by.y = , suffixes = c (".fetal" , ".adult"))
```
...
Question
-- where is the 'batch information' ? is it a column in the dataset ?
```{r}
##### 5
# ComBat
